		...
		if(Physics.Raycast(transform.position, Vector3.down, out hit, 1000.0f))
		{
			...
			if(hit.collider.gameObject.layer == LayerMask.NameToLayer("Ground"))//The Viking Village terrain mesh (terrain_near_01) is set to the Ground layer.
			{
				...
				if(materialIndex != -1)
				{
					...
					if(material.name == "Terraim")//This texture name is specific to the terrain mesh in the Viking Village scene.
					{
						if(m_Debug)
						...
						m_Grass = maskPixel.a; // * Mathf.Min(spec2Pixel.a * specMultiplier, 0.9f);//Only the sand texture has water, so we multiply by the mask pixel alpha value.
						m_Gravel = 1.0f - maskPixel.a;
